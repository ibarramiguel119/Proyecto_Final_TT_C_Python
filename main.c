...
osStaticSemaphoreDef_t MotorSemaphoreControlBlock;
const osSemaphoreAttr_t MotorSemaphore_attributes = {
  .name = "MotorSemaphore",
  .cb_mem = &MotorSemaphoreControlBlock,
  .cb_size = sizeof(MotorSemaphoreControlBlock),
};
/* USER CODE BEGIN PV */


// Declaración global para las colas de los motores
// Declaración global para las colas de los motores
osMessageQueueId_t motor1Queue;
osMessageQueueId_t motor2Queue;
osMessageQueueId_t motor3Queue;
osMessageQueueId_t motor4Queue;
osMessageQueueId_t motor5Queue;
osMessageQueueId_t confirmQueue;
...
uint32_t milimetros_a_pasos_m3(float milimetros) {
    // Calcular el número de pasos necesarios para mover la distancia en milímetros
    float pasos_por_mm = 200.0 / 8.0; // 200 pasos por 8 mm
    return (uint32_t)(fabs(milimetros) * pasos_por_mm);
}
...
void A4988_q1(){
    ...
    HAL_GPIO_WritePin(GPIOB, MS1_PIN_q1, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOB, MS2_PIN_q1, GPIO_PIN_SET);
}
...
void mover_motorq1_rad(float radianes){
    ...
    int pasos = (int)((radianes / (2 * M_PI)) * 3200);
...
void mover_motorq3_mm(float milimetros){
    ...
    uint32_t pasos = milimetros_a_pasos_m3(milimetros);
    int nuevo_paso = pasos;
...
///Funcion que recibe datos del puerto UART por callback
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1)
    {
        ...
        else
        {
        	...
        	HAL_UART_Transmit(&huart3, (uint8_t *)"Buffer recibido: ", 17, HAL_MAX_DELAY);
        	HAL_UART_Transmit(&huart3, buffer, bufferIndex, HAL_MAX_DELAY); // Envía el contenido del buffer
...
///Funccion que procesa los datos de uart
...
void processBuffer(uint8_t *buffer, uint16_t length)
{
    ...
    char msgp[50]; // Buffer para el


    // Convertir las cadenas a números flotantes
    MotorCommand cmd1 = { atof(q1) }; // Motor 1 (radianes)
    MotorCommand cmd2 = { atof(q2) }; // Motor 2 (milímetros)
    MotorCommand cmd3 = { atof(q3) }; // Motor 3 (milímetros)
    MotorCommand cmd4 = { atof(q4) }; // Motor 4 (radianes)
    MotorCommand cmd5 = { atof(q5) }; // Motor 5 (radianes)

    sprintf(msgp, "Prueba de valor que se asigna a CMD1:%.4f\n", cmd1.value); // %.4f formatea a 4 decimales
    HAL_UART_Transmit(&huart3, (uint8_t*)msgp, strlen(msgp), HAL_MAX_DELAY);

    sprintf(msgp, "Prueba de valor que se asigna a CMD2:%.4f\n", cmd2.value); // %.4f formatea a 4 decimales
    HAL_UART_Transmit(&huart3, (uint8_t*)msgp, strlen(msgp), HAL_MAX_DELAY);

    sprintf(msgp, "Prueba de valor que se asigna a CMD3:%.4f\n", cmd3.value); // %.4f formatea a 4 decimales
    HAL_UART_Transmit(&huart3, (uint8_t*)msgp, strlen(msgp), HAL_MAX_DELAY);

    char msg[] = "The data is send succesfull...\r\n";  // Message specific to Motor 1
    char msg1[] = "The data cant send it was a problem...\r\n";  // Message specific to Motor 1

    char msg2[] = "The data2 is send succesfull...\r\n";  // Message specific to Motor 1
    char msg3[] = "The data2 cant send it was a problem...\r\n";  // Message specific to Motor 1

    char msg4[] = "The data3 is send succesfull...\r\n";  // Message specific to Motor 1
    char msg5[] = "The data3 cant send it was a problem...\r\n";  // Message specific to Motor 1


    // Variables para los resultados
    osStatus_t status1, status2, status3, status4, status5;


    // Enviar comandos a las respectivas colas
    status1 = osMessageQueuePut(motor1Queue, &cmd1, 0, 0);
    if (status1 == osOK) {
    ...
    status2 = osMessageQueuePut(motor2Queue, &cmd2, 0, 0);
    if (status2 == osOK) {
    ...
    status3 = osMessageQueuePut(motor3Queue, &cmd3, 0, 0);
    if (status3 == osOK) {
    	HAL_UART_Transmit(&huart3, (uint8_t*)msg4, strlen(msg2), HAL_MAX_DELAY);

    } else {
    	HAL_UART_Transmit(&huart3, (uint8_t*)msg5, strlen(msg3), HAL_MAX_DELAY);
    }

    status4 = osMessageQueuePut(motor4Queue, &cmd4, 0, 0);
    if (status4 == osOK) {
    ...
    status5 = osMessageQueuePut(motor5Queue, &cmd5, 0, 0);
    if (status5 == osOK) {
...
void Detener_Sistema(void){
	  osThreadSuspend(Mover_Motor1Handle);
	  osThreadSuspend(Mover_Motor2Handle);
	  osThreadSuspend(Mover_Motor3Handle);
	  //osThreadSuspend(Mover_Motor4Handle);
	  //osThreadSuspend(Mover_Motor5Handle);
}

void Reanudar_Sistema()
{
    osThreadResume(Mover_Motor1Handle);
    osThreadResume(Mover_Motor2Handle);
    osThreadResume(Mover_Motor3Handle);
    // Optionally resume other threads
}
...
/* Configure the peripherals common clocks */
  ...
  motor1Queue = osMessageQueueNew(10, sizeof(MotorCommand), NULL);
  motor2Queue = osMessageQueueNew(10, sizeof(MotorCommand), NULL);
  motor3Queue = osMessageQueueNew(10, sizeof(MotorCommand), NULL);
  motor4Queue = osMessageQueueNew(10, sizeof(MotorCommand), NULL);
  motor5Queue = osMessageQueueNew(10, sizeof(MotorCommand), NULL);
  confirmQueue = osMessageQueueNew(10, sizeof(int), NULL);
  ...
  Mover_Motor1Handle = osThreadNew(Mover_Motor_1, NULL, &Mover_Motor1_attributes);

  /* creation of Mover_Motor2 */
  Mover_Motor2Handle = osThreadNew(Mover_Motor_2, NULL, &Mover_Motor2_attributes);

  /* creation of Mover_Motor3 */
  Mover_Motor3Handle = osThreadNew(Mover_Motor_3, NULL, &Mover_Motor3_attributes);

  /* creation of Mover_Motor4 */
  Mover_Motor4Handle = osThreadNew(Mover_Motor_4, NULL, &Mover_Motor4_attributes);

  /* creation of Mover_Motor5 */
  Mover_Motor5Handle = osThreadNew(Mover_Motor_5, NULL, &Mover_Motor5_attributes);

  /* creation of Envar_datos_Uar */
  Envar_datos_UarHandle = osThreadNew(Enviar_datos_confirmaccion, NULL, &Envar_datos_Uar_attributes);

  /* creation of Recibir_datos_u */
  Recibir_datos_uHandle = osThreadNew(Recibir_datos_app, NULL, &Recibir_datos_u_attributes);
...
void Mover_Motor_1(void *argument)
{
  ...
  for (;;)
  {
    // Obtener un mensaje desde la cola motor1Queue
    if (osMessageQueueGet(motor1Queue, &command, NULL, osWaitForever) == osOK)
    {
      mover_motorq1_rad(command.value); // Motor 1 espera radianes
      int motorId = 1;
      osMessageQueuePut(confirmQueue, &motorId, 0, 0);

    }
    osDelay(1000); // Espera 1 segundo antes de repetir
  }
  /* USER CODE END 5 */
}
...
void Mover_Motor_2(void *argument)
{
    ...
    MotorCommand command1;

    // Bucle infinito
    for (;;)
    {

        // Obtener un mensaje desde la cola motor2Queue
         if (osMessageQueueGet(motor2Queue, &command1, NULL, osWaitForever) == osOK) {

            // Llamar a la función que mueve el motor con el valor recibido (en milímetros)
            mover_motorq2_mm(command1.value); // Motor 2 espera milímetros
            int motorId = 2;
            osMessageQueuePut(confirmQueue, &motorId, 0, 0);
        }
        osDelay(1000); // Espera 1 segundo antes de repetir
...
void Mover_Motor_3(void *argument) {
    ...
    MotorCommand command2;

    /* Infinite loop */
    for (;;) {
        // Esperar un mensaje en la cola de motor3
        if (osMessageQueueGet(motor3Queue, &command2, NULL, osWaitForever) == osOK) {
            // Mover el motor con el valor recibido
            mover_motorq3_mm(command2.value); // Motor 3 espera milímetros
            int motorId = 3;
            osMessageQueuePut(confirmQueue, &motorId, 0, 0);
        }
...
void Mover_Motor_4(void *argument) {
  ...
  MotorCommand command3;
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_4); // Asegúrate de que el PWM esté activo
  for (;;) {
	  if(osMessageQueueGet(motor4Queue, &command3, NULL, osWaitForever) == osOK){
		 TIM2->CCR4 = radianes_a_valor(command3.value); // Mueve el servo al ángulo deseado
		 int motorId = 4;
		 osMessageQueuePut(confirmQueue, &motorId, 0, 0);
	  }
      osDelay(1000); // Espera 1 segundo antes de repetir
...
void Mover_Motor_5(void *argument)
{
  ...
  MotorCommand command4;
  // Asegúrate de que el PWM del canal 2 está habilitado
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_2);

  /* Infinite loop */
  for(;;)
  {
	 if(osMessageQueueGet(motor5Queue, &command4, NULL, osWaitForever) == osOK){
		 TIM2->CCR2 = radianes_a_valor(command4.value); // Mueve el servo al ángulo deseado
		 int motorId = 5;
		 osMessageQueuePut(confirmQueue, &motorId, 0, 0);
	 }
	 osDelay(1000); // Espera 1 segundo antes de repetir
...
void Tarea_administradora(void *argument)
{
  ...
  int motorId;                // Para almacenar los identificadores de los motores que terminan
  uint8_t confirmation = 1;    // Confirmación como número (vamos a convertirlo a string)

  /* Infinite loop */
  for(;;)
  {
    // Verificar confirmaciones de los motores (esperar hasta que lleguen)
    for (int i = 0; i < 5; i++) { // Esperamos confirmación de 3 motores
        if (osMessageQueueGet(confirmQueue, &motorId, NULL, osWaitForever) == osOK) {
            // Si se recibe un motorId, procesar según el caso necesario
        }
    }

    // Convertir el número de confirmación a string
    char confirmationStr[2];  // Solo 1 carácter y el terminador nulo
    snprintf(confirmationStr, sizeof(confirmationStr), "%d", confirmation);

    // Enviar la confirmación como string por UART
    HAL_UART_Transmit(&huart1, (uint8_t*)confirmationStr, strlen(confirmationStr), HAL_MAX_DELAY);

    char msg_confirmation[] = "The data '1' was sent successfully...\r\n";  // Mensaje específico para el motor
    HAL_UART_Transmit(&huart3, (uint8_t*)msg_confirmation, strlen(msg_confirmation), HAL_MAX_DELAY);

    // Agregar un retraso antes de reiniciar el ciclo (si es necesario)
    osDelay(100);

    // Detener el sistema (si esto es lo que se necesita hacer)
    //Detener_Sistema();
  }
...
void Boton_interrupt(void *argument)
{
    ...
    GPIO_PinState botonEstadoAnterior = GPIO_PIN_RESET;  // Para almacenar el estado previo del botón
    GPIO_PinState botonEstadoActual;  // Para almacenar el estado actual del botón
